...
impl Analysis for UnsafeTraitSafeMethod {
    ...
    fn new() -> Self { UnsafeTraitSafeMethod{ has_unsafe: false } }
    ...
    fn run_analysis<'a, 'tcx>(cx: &LateContext<'a, 'tcx>, fn_info: &'a FnInfo)
        ...
        let mut analysis: Self = Analysis::new();
        if unsafe_trait_visitor.has_unsafe { analysis.set(); }
        analysis
    }
}
...
impl <'a, 'tcx> SafeMethodsInUnsafeTraits<'a, 'tcx> {
    pub fn new(cx: &'a LateContext<'a, 'tcx>) -> Self {
        SafeMethodsInUnsafeTraits { cx, has_unsafe: false}
    }
}
