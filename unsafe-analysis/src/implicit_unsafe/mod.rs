...
pub fn run_sources_analysis<'a, 'tcx>(cx: &LateContext<'a, 'tcx>
                                      , fns: &Vec<NodeId>, optimistic: bool, restricted: bool)
                                      -> Vec<UnsafeInBody> {
    ...
    let implicit_external: FxHashMap<DefId,UnsafeInBody> =
        deps::load(cx, &external_calls, optimistic, restricted);
...
fn has_unsafe<'a, 'tcx>(cx: &LateContext<'a, 'tcx>, fn_id: DefId, restricted: bool) -> bool {
    if restricted {
...
impl<'a, 'b, 'tcx:'a+'b>  CallsVisitor<'a, 'b, 'tcx> {
    ...
    fn new(cx: &'a LateContext<'a, 'tcx>,
        ...
        CallsVisitor { cx, mir, fn_ctx, call_graph, with_unsafe, external_calls, optimistic, restricted, depth}
    }
...
impl<'a, 'b, 'tcx:'a+'b> Visitor<'tcx> for CallsVisitor<'a,'b,'tcx> {
    ...
    fn visit_mir(&mut self, mir: &Mir<'tcx>) {
        ...
        if has_unsafe(self.cx, self.fn_ctx.def_id, self.restricted) {
            ...
            if !self.with_unsafe.contains(self.fn_ctx) {
                ...
                match func.ty(&self.mir.local_decls, self.cx.tcx).sty {
                    TyKind::FnDef(callee_def_id, callee_substs) => {
                        if deps::is_library_crate(&self.cx.tcx.crate_name(callee_def_id.krate).to_string()) {
                            // do nothing
                        } else {
