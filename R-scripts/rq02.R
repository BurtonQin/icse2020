...
cdf_filename <- "~/work/unsafe-analysis-data/paper/rq02_all_cdf.eps"
...
cdf_filename <- "~/work/unsafe-analysis-data/paper/rq02_most_downloaded_cdf.eps"

res <- read.table( file="~/unsafe_analysis/analysis-data/research-questions-90-percent/rq07_opt"
                   , header=FALSE
                   , sep='\t'
                   , comment.char = "#"
                   , col.names=c("crate", "no"))

res1 <- read.table( file="~/unsafe_analysis/analysis-data/research-questions-90-percent/rq07_pes"
                    , header=FALSE
                    , sep='\t'
                    , comment.char = "#"
                    , col.names=c("crate", "no"))
...
first_y <- ceiling(min_y*10)/10 + 0.1
summary <- quantile(res1$no, c(.90,.95,.99))
x_max <- summary["95%"]
...
options(digits = 4)
write(y1*100,file="~/work/unsafe-analysis-data/paper/rq02_most_downloaded_safe_lower.txt")
write(y0*100,file="~/work/unsafe-analysis-data/paper/rq02_most_downloaded_safe_upper.txt")

opt_max <- max(res$no)
write(format(opt_max, big.mark=","),file="~/work/unsafe-analysis-data/paper/rq02_most_downloaded_opt_max.txt")
cons_max <- max(res1$no)
write(format(cons_max, big.mark=","),file="~/work/unsafe-analysis-data/paper/rq02_most_downloaded_cons_max.txt")